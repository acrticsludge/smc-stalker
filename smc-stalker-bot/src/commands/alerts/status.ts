  ...
  defineCommand({
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
      ...
      const lines = configs.map((c, i) => {
        ...
        const trackedNations = c.type === 'friendly' && Object.keys(c.nation_pings).length > 0
          ? ` | tracks: ${Object.keys(c.nation_pings).join(', ')}`
          : '';
        const time =
          c.schedule_times.length > 0
            ? ` at ${c.schedule_times.map((t) => formatTimeGMT(t)).join(', ')}`
            : ' (immediate)';
        return `\`[${idx}]\` **${type}**${nation}: ${status} → ${channel}${role}${trackedNations}${time}`;
      });
